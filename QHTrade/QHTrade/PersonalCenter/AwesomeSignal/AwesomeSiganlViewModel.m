...
@implementation AwesomeSiganlViewModel
...
-(void)initialize {
    ...
    [[[self.refreshDataCommand executionSignals]switchToLatest] subscribeNext:^(NSMutableArray * array) {
        ...
        for (NSDictionary *dic in array) {
            AwesomeSignalModel *model = [AwesomeSignalModel mj_objectWithKeyValues:dic];
        ...
        _refreshDataCommand = [[RACCommand alloc]initWithSignalBlock:^RACSignal *(id input) {
            return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
                ...
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    ...
                    QHRequestModel * model = [QHRequest getDataWithApi:@"/notifyMsg/okami/trade" withParam:nil error:&error];
