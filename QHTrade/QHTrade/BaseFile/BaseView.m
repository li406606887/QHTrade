...
- (instancetype)initWithViewModel:(id<BaseViewModelProtocol>)viewModel {
    ...
    if (self) {
        [self setupViews];
        [self bindViewModel];
        [self addSubview:self.loadingFail];
        [self addSubview:self.promptLabel];
        [self.loadingFail mas_makeConstraints:^(MASConstraintMaker *make) {
            make.center.equalTo(self);
            make.size.mas_offset(CGSizeMake(107, 94));
        }];
        [self.promptLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerX.equalTo(self);
            make.top.equalTo(self.loadingFail.mas_bottom).with.offset(5);
