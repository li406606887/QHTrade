...
- (void)drawRect:(CGRect)rect {
        ...
        for (NSInteger i = 0; i < self.yValueArray.count; i++) {
            ...
            if (i == self.yValueArray.count-1) {
                ...
                if (!_isShowLabel) {
                    ...
                    CGRect strRect = CGRectMake(startPoint.x-strSize.width/2,startPoint.y-strSize.height-10,strSize.width,strSize.height);
                    if (i == 0) {
                        self.firstStrFrame = strRect;
    ...
    if(self.isLongPress) {
        ...
        if(nowPoint >= 0 && nowPoint < [self.yValueArray count]) {
            ...
            CGSize timeSize = [[NSString stringWithFormat:@"%@",self.xTitleArray[nowPoint]] sizeWithAttributes:timeAttr];
            ...
            //画选中的数值
            [[NSString stringWithFormat:@"%@",self.xTitleArray[nowPoint]] drawAtPoint:CGPointMake(drawPoint.x, drawPoint.y) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12],NSForegroundColorAttributeName:[UIColor blackColor]}];
            
            
            // 判断是不是小数
            if ([self isPureFloat:[num floatValue]]) {
                [[NSString stringWithFormat:@"%.4f", [num floatValue]] drawAtPoint:CGPointMake(drawPoint.x, drawPoint.y+15) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12],NSForegroundColorAttributeName:[UIColor blackColor]}];
            }
            else {
                [[NSString stringWithFormat:@"%.0f", [num floatValue]] drawAtPoint:CGPointMake(drawPoint.x, drawPoint.y+15)withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12],NSForegroundColorAttributeName:[UIColor blackColor]}];
